...
namespace EZData
{
    ...
    public class Context : IBindingPathTarget, IContext
    {
        ...
        internal static Delegate NodeToDelegate(object node, string path)
        {
            ...
            var reflectionProperty = NDataReflectionUtils.GetMethod(node.GetType(), path);
            ...
            return NDataReflectionUtils.CreateDelegate(typeof(Command), node, reflectionProperty);
        }
        ...
        internal static Property<int> NodeToEnumProperty(object node, string path)
        {
            ...
            var reflectionProperty = NDataReflectionUtils.GetProperty(node.GetType(), path + "Property");
            if (reflectionProperty != null)
            {
                return reflectionProperty.GetValue(node, null) as Property<int>;
            }

            reflectionProperty = NDataReflectionUtils.GetProperty(node.GetType(), path);
            if (reflectionProperty != null)
            {
                var value = reflectionProperty.GetValue(node, null);
                if (value != null)
                {
                    if (NDataReflectionUtils.IsEnum(value.GetType()))
                    {
        ...
        internal static Collection NodeToCollection(object node, string path)
        {
            ...
            var reflectionProperty = NDataReflectionUtils.GetProperty(node.GetType(), path);
        ...
        internal static T Find<T>(object node, string path, IBinding binding, Converter<T> converter)
        {
            ...
            if (int.TryParse(nodePropertyName, out collectionItemIndex) && node is Collection)
            {
                ...
                var varSubNode = collection.GetItemPlaceholder(collectionItemIndex);
                varSubNode.AddBindingDependency(binding);
                var valueReflectionProperty = NDataReflectionUtils.GetProperty(varSubNode.GetType(), "Value");
            ...
            var varReflectionProperty = NDataReflectionUtils.GetField(node.GetType(), nodePropertyName + VariableContextPostfix);
            ...
            var reflectionProperty = NDataReflectionUtils.GetProperty(node.GetType(), nodePropertyName);
            var subNode = (reflectionProperty == null) ? null : reflectionProperty.GetValue(node, null);
			if (subNode == null)
			{
				var nodeReflectionField = NDataReflectionUtils.GetField(node.GetType(), nodePropertyName);
		...
		internal static Property<T> FindProperty<T>(object node, string path, IBinding binding)
        {
            ...
            if (pointPos < 0)
            {
				...
				var reflectionField = NDataReflectionUtils.GetField(node.GetType(), path);
            ...
            var nodeReflectionProperty = NDataReflectionUtils.GetProperty(node.GetType(), nodePropertyName);
