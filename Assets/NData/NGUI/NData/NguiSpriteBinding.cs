...
public class NguiSpriteBinding : NguiBinding
{
	...
	protected UISprite _UiSpriteReceiver;
	
	public override void Awake()
	{
		base.Awake();

        if (!_properties.ContainsKey(typeof(string)))
		_properties.Add(typeof(string), null);
#if NGUI_2
        if (!_properties.ContainsKey(typeof(UIAtlas.Sprite)))
		_properties.Add(typeof(UIAtlas.Sprite), null);
#else
        if (!_properties.ContainsKey(typeof(UISpriteData)))
	...
	protected override void Bind()
	{
		...
		if (context == null)
		{
            Debug.LogWarning("NguiSpriteBinding.UpdateBinding - context is null "  + NguiUtils.GetPathDepth(Path));
			return;
		}
